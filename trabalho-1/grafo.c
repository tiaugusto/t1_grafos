...
typedef struct vert {
    char *nome;              /* nome (string duplicada)                      */
...
grafo *le_grafo(FILE *f)
{
    ...
    while (fgets(linha, sizeof(linha), f)) {
        ...
        /* vertex or edge?  procura marcador "--"  */
        char *marc = strstr(p, "--");
        if (!marc) {                /* vértice isolado */
            cria_vertice(g, p);
            continue;
        }
        ...
        int  peso = 1;
        /* split em três partes: v1, v2, [peso] */
                ...
                int *dist, int *componente, int comp_id)
{
    ...
    while (ini < fim) {
        ...
        for (adj_t *a = g->v[u].cab; a; a = a->prox) {
            ...
            if (dist[v] == -1) {
                dist[v] = dist[u] + 1;   /* distância sem peso           */
...
unsigned int diametro_componente(const grafo *g,
                                        const unsigned int *vlist,
                                        unsigned int tam)
{
    unsigned int n = g->n_vertices;
    int *dist = malloc(n * sizeof(int));
    unsigned int diam = 0, *fila = malloc(n * sizeof(unsigned int));

    for (unsigned int idx = 0; idx < tam; ++idx) {
        unsigned int s = vlist[idx];
        for (unsigned int i = 0; i < n; ++i) dist[i] = -1;
        ...
        while (ini < fim) {
            ...
            for (adj_t *a = g->v[u].cab; a; a = a->prox) {
                ...
                if (dist[v] == -1) {
                    dist[v] = dist[u] + 1;
                    fila[fim++] = v;
                    if ((unsigned)dist[v] > diam) diam = dist[v];
                }
            }
        }
    }
...
char *vertices_corte(grafo *g)
{
    ...
    if (qtd == 0) { free(arts); return xstrdup(""); }
...
char *arestas_corte(grafo *g)
{
    ...
    const unsigned int comps0 = componentes(g, -1, -1, -1);
    /* pior caso: todas as arestas são pontes                               */
    typedef struct { unsigned u, v; } par;
    par *pontes = malloc(g->n_arestas * sizeof(par));
    unsigned int qtd = 0;

    for (unsigned int u = 0; u < n; ++u)
        for (adj_t *a = g->v[u].cab; a; a = a->prox)
            if (u < a->dest) {                     /* evita duplicar arestas */
                if (componentes(g, -1, (int)u, (int)a->dest) > comps0) {
                    unsigned int v = a->dest;
                    if (strcmp(g->v[u].nome, g->v[v].nome) < 0)
                        pontes[qtd++] = (par){u, v};
                    else
                        pontes[qtd++] = (par){v, u};
                }
    ...
    if (qtd == 0) { free(pontes); return xstrdup(""); }

    /* ordenação alfabética */
    qsort_r(pontes, qtd, sizeof(par),
            (int(*)(const void*,const void*,void*))cmp_pair, g);

    size_t len = 0;
    for (unsigned i = 0; i < qtd; ++i)
        len += strlen(g->v[pontes[i].u].nome) + 1 +
               strlen(g->v[pontes[i].v].nome) + 1;

    char *out = malloc(len); out[0] = '\0';
    for (unsigned i = 0; i < qtd; ++i) {
        strcat(out, g->v[pontes[i].u].nome); strcat(out, " ");
        strcat(out, g->v[pontes[i].v].nome);
        if (i + 1 < qtd) strcat(out, " ");
    }
    free(pontes); return out;
}